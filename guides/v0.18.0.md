...
- Shapecasts are similar to raycasts, except instead of detecting collision along a line they sweep a
  whole shape through the scene and find anything it touches.
- They're very helpful for player locomotion and projectile hit tests.

### CCD

- Normally the physics engine will compute a new position for a collider, and then detect and
  resolve any collisions at the new location.
- This usually works fine, but fast-moving objects can pass through walls if they move fully through
  through them during one physics step.
- CCD (continuous collision detection) is a technique that solves this problem by checking for
  collisions along the object's whole path, instead of just checking the final position.  
- It's great for fast moving projectiles.
- Simply use `Collider:setContinuous(true)` on any colliders that need CCD.
...
- `enter` is fired when 2 objects start touching.  This could be used to play sounds or spawn
  particles.
...
other vertex data like UVs, which lets you paint on a Model!
...
In addition to TTF fonts, LÖVR now supports fonts in the BMFont format.  This is an atlas of glyph
images.  Compared to the existing MSDF fonts, BMFonts look better and are more efficient for 2D
text, making them a great fit for rendering text on the new quad layers.  Since the image atlas can
contain anything, they can also be used for icons/emoji.  However, they don't support scaling like
MSDF fonts do, so they can get blurry when rendering text in 3D space.
...
- Udinanon also made a spherical harmonics viewer: [Spherical Harmonics and surfaces, an attempt](https://martinoshelf.neocities.org/spherical-harmonics-lovr)
