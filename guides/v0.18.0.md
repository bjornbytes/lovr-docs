...
for fast moving projectiles.  Simply use `Collider:setContinuous(true)` on any colliders that need
CCD.
...
text, making them a great fit for rendering text on the new `Layer` objects.  Since the image atlas
can contain anything, they can also be used for icons/emoji.  However, they don't support scaling
like MSDF fonts do, so they can get blurry when rendering text in 3D space.
...
correctly sent to `texture2DMSAA` shader variables.  Additionally, `Pass:setCanvas` accepts a set of
`resolve` attachments that can be used to do custom MSAA resolves.
