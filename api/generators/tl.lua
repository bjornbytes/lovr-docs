...
return function(api)
  ...
  local function normalizeUnionParts(parts)
    ...
    elseif totalTables > 1 then
      if #tableNon > 0 then
        addUnique(normalized, seenNormalized, 'AnyTable')
      else
  ...
  local function convertTypeString(typeStr)
    ...
    typeStr = typeStr:gsub('%f[%w]table%f[^%w]', 'AnyTable')
  ...
  preamble[#preamble + 1] = 'global type AnyTable = {any:any}'
  preamble[#preamble + 1] = 'global type lightuserdata = userdata'
  preamble[#preamble + 1] = ''
